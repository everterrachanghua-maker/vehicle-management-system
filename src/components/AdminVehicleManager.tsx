...
export default function AdminVehicleManager() {
  ...
  const handleAddVehicle = async (e: any) => {
    ...
    try {
      let imgUrl = "";
      if (photoFile) imgUrl = await handleFileUpload(photoFile, plate);
  ...
  const handleUpdateVehicle = async (e: any) => {
    ...
    try {
      ...
      if (newPhotoFile) finalImgUrl = await handleFileUpload(newPhotoFile, plate);
    ...
    <div className="space-y-8 pb-20">
      ...
      <section className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleAddVehicle} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
          ...
          <button disabled={isUploading} className="bg-[#0f172a] text-white h-[52px] rounded-2xl font-bold transition-all shadow-lg flex items-center justify-center disabled:opacity-70">{isUploading ? <Loader2 className="animate-spin" /> : "確認入庫"}</button>
        </form>
      </section>
                  ...
                  <React.Fragment key={v.id}>
                    <tr className={`${rowBgColor} transition-colors duration-300 group`}>
                      ...
                      <td className="px-4 py-5 text-center">
                        <select value={v.status} onChange={async (e) => await updateDoc(doc(db, "vehicles", v.id), { status: e.target.value })} className={`text-[10px] font-bold px-2 py-1 rounded-lg border-none outline-none cursor-pointer ${v.status === 'available' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                          <option value="available">可填寫</option><option value="maintenance">維修中</option>
                        </select>
                      </td>
                          ...
                          <div className="m-6 bg-white rounded-2xl border border-slate-200 shadow-inner overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                            <table className="w-full text-xs text-left">
                              <thead className="bg-slate-50 text-slate-400 font-bold border-b">
                                <tr><th className="px-6 py-3">日期 / 時間</th><th className="px-6 py-3">填報人</th><th className="px-6 py-3 text-center">工作地點</th><th className="px-6 py-3 text-center">里程 (起 → 止)</th><th className="px-6 py-3 text-center">單次里程</th><th className="px-6 py-3 text-center">狀況</th><th className="px-6 py-3 text-right">操作</th></tr>
                              </thead>
                              <tbody className="divide-y divide-slate-100">
                                {vRecords.length === 0 ? <tr><td colSpan={7} className="p-8 text-center text-slate-300 italic">尚無行駛紀錄</td></tr> : vRecords.map(rec => (
      ...
      {editingVehicle && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleUpdateVehicle} className="bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 bg-[#0f172a] text-white flex justify-between items-center"><h3 className="font-bold text-lg">編輯車輛資訊</h3><button type="button" onClick={() => setEditingVehicle(null)}><X/></button></div>
            <div className="p-8 space-y-4">
              <div className="flex flex-col items-center gap-3 mb-4">
                <div className="relative group"><div className="w-24 h-24 rounded-2xl bg-slate-50 border-2 border-slate-200 overflow-hidden flex items-center justify-center">{editingVehicle.imgUrl ? <img src={editingVehicle.imgUrl} className="w-full h-full object-cover" /> : <Car size={32} className="text-slate-200" />}</div><label htmlFor="edit_photo" className="absolute -bottom-2 -right-2 w-8 h-8 bg-emerald-500 text-white rounded-xl flex items-center justify-center shadow-lg cursor-pointer hover:bg-emerald-600 transition-all border-2 border-white"><Camera size={14} /></label><input name="edit_photo" type="file" accept="image/*" className="hidden" id="edit_photo" /></div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">更換照片</p>
              </div>
              ...
              <InputGroup label="最初里程" type="number" value={editingVehicle.initialOdo} onChange={(e:any)=>setEditingVehicle({...editingVehicle, initialOdo: e.target.value})} />
              ...
              <button disabled={isUpdating} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-xl hover:bg-emerald-700 transition-all disabled:opacity-70">{isUpdating ? <Loader2 className="animate-spin" /> : <><Save size={20} /> 儲存變更</>}</button>
            </div>
          </form>
        </div>
      )}
