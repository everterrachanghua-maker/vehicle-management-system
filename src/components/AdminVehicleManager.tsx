...
import { 
  ...
  Droplets, Trash2, Edit3, Camera, Loader2, PlusCircle, X, Save, FileText, MapPin, User, Clock
} from 'lucide-react';
  ...
  const handleUpdateVehicle = async (e: any) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "vehicles", id), {
        name, plate: plate.toUpperCase(), initialOdo: Number(initialOdo), 
        contractOdo: Number(contractOdo), imgUrl: finalImgUrl
      });
  ...
  const handleUpdateRecord = async (e: any) => {
    ...
    try {
      await updateDoc(doc(db, "records", id), {
        ...
        mileageDiff: Number(endOdo) - Number(startOdo),
        location,
        userName,
        date,
        time
      ...
      <section className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleAddVehicle} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
          <div className="space-y-2 text-center">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">車輛縮圖</label>
            <div className="relative group mx-auto">
              <input name="photo" type="file" accept="image/*" className="hidden" id="vehicle-photo" onChange={handleFileChange} />
              <label htmlFor="vehicle-photo" className="flex items-center justify-center h-[52px] w-[52px] mx-auto bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer group-hover:border-emerald-500 transition-all overflow-hidden">
                {previewUrl && !editingVehicle ? <img src={previewUrl} className="w-full h-full object-cover" /> : <Camera size={20} className="text-slate-400" />}
              </label>
            </div>
          </div>
            ...
            <div className="p-8 space-y-6">
              <div className="flex flex-col items-center gap-4 py-2">
                <div className="relative group">
                  ...
                  <label htmlFor="edit_photo" className="absolute -bottom-2 -right-2 w-10 h-10 bg-emerald-500 text-white rounded-2xl flex items-center justify-center shadow-lg cursor-pointer border-4 border-white"><Camera size={18} /></label>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form onSubmit={handleUpdateRecord} className="bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
             ...
             <div className="p-8 space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-4">
                   <InputGroup label="日期" type="date" value={editingRecord.date} onChange={(e:any)=>setEditingRecord({...editingRecord, date: e.target.value})} />
                   <InputGroup label="時間" type="time" value={editingRecord.time} onChange={(e:any)=>setEditingRecord({...editingRecord, time: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <InputGroup label="填報人" value={editingRecord.userName} onChange={(e:any)=>setEditingRecord({...editingRecord, userName: e.target.value})} />
                   <InputGroup label="地點" value={editingRecord.location} onChange={(e:any)=>setEditingRecord({...editingRecord, location: e.target.value})} />
                </div>
  ...
  return (
    <div className="space-y-1">
      ...
      <input 
        ...
        className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 ring-emerald-500 font-bold transition-all text-sm" 
        required 
      />
    </div>
  );
