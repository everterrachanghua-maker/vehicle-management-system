...
interface Vehicle {
  ...
  plate: string;
  current_odo: number;
  status?: 'available' | 'maintenance'; // 新增狀態型別
}
      ...
      {showAdd && isAdmin && (
        <form onSubmit={handleAddVehicle} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xl space-y-4 mb-8 animate-in fade-in slide-in-from-top-4">
          ...
          <div className="flex justify-end">
            <button disabled={isSubmitting} className="bg-[#0f172a] text-white px-6 py-2 rounded-xl font-bold text-sm">
              ...
              >
                <div className="flex items-center gap-5">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors
                    ${isMaintenance ? 'bg-slate-100 text-slate-400' : 'bg-slate-50 text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600'}`}>
                    <Car size={24} />
                  </div>
                  <div>
                ...
                <div className="text-right">
                  {/* 狀態標籤 */}
                  <div className={`text-[10px] font-black px-2 py-0.5 rounded-full mb-2 inline-flex items-center gap-1 uppercase tracking-tighter
                    ${isMaintenance ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>
                    {isMaintenance ? (
                  ...
                  <p className="text-sm font-black text-slate-700">
                    {(v.current_odo || 0).toLocaleString()} km
                  </p>
